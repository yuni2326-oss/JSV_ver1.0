...
def fig_a4_landscape(data: Path, figs: Path):
    ...
    (0.006–0.47). Δχ²로 그리면 Δχ²=3.84(95 %)·11.8 문턱선이 골의 폭을 직접 읽게 해준다.
    ...
    for ri, s_t in enumerate(s_cases):
        for ci, xi_t in enumerate(xi_cases):
            ...
            cs_ = ax.contour(xi_g, s_g * 100, Z.T, levels=[3.84, 11.8],
                             colors=["crimson", "0.35"], linewidths=[1.6, 1.0])
            ax.clabel(cs_, fmt={3.84: "3.84", 11.8: "11.8"}, fontsize=6)
